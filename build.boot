...
(task-options! pom {:project 'edge
                    :version +version+
                    :url ""
                    :description "The Lambone project"
                    :license {}})
...
(def frontend-options
  {:env {:source-paths #{"sass" "src/frontend" "env/dev" "test/frontend"}
  ...
  (let [type (or type :prod)
    ...
    (let [cljs (resolve 'adzerk.boot-cljs/cljs)]
      (comp (with-pass-thru _
              ...
              (util/warn "Env :dependencies:\n%s\n" (string/join "\n" (:dependencies (get-env)))))
            (apply cljs (flatten (seq (:cljs options))))
            (if-not out
              (sift :include #{#"edge.out"} :invert true)
              identity)))))
...
(deftask test-frontend
  ...
  (let [type (or type :prod)
    ...
    (let [test-cljs (resolve 'crisptrutski.boot-cljs-test/test-cljs)]
      (comp
       (with-pass-thru _
         (util/info "Testing %s frontend, prepend with watch for auto testing...\n" type )
